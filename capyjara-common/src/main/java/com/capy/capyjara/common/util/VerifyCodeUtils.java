...
public class VerifyCodeUtils {
     ...
     * 特殊字符
     */
    private  static final char[] SYMBOLS_SPECIAL_CHAR = "!@#$%&*+=,.".toCharArray();
     ...
     * @return 验证码
     */
    private static String generateSpecialChar(int length){
        char[] lib = SYMBOLS_SPECIAL_CHAR;
        char[] nonceChars = new char[length];
        for (int index = 0; index < nonceChars.length; index++) {
            nonceChars[index] = lib[RANDOM.nextInt(lib.length)];
        }
